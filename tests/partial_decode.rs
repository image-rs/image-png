...
/// Regression test for a scenario that was originally discovered using
/// `cargo fuzz run buf_independent` and discussed at
...
///   in `fuzz/fuzz_targets/buf_independent.rs`) always results in
///   `Err(Format(FormatError { inner: CorruptFlateStream { err: InsufficientInput } }))`
/// * Decoding streaming input (see `intermittent_eofs_reader` in
///   `fuzz/fuzz_targets/buf_independent.rs`) may unexpectedly miss this error
///   and return `Ok(_)` unless special care is taken (see below)
///
/// The input file here has an IDAT payload that covers all image rows, but
/// is truncated without properly terminating the zlib stream.  Consistently
/// detecting this error condition requires always calling
/// `ZlibStream::finish_compressed_chunks` (even though those remaining
/// decompressed bytes do not always correspond to any actual image rows).
#[test]
fn test_partial_decode_with_unterminated_redundant_zlib_tail() {
    ...
    // Write the remaining input bytes and verify that `InsufficientInput`
    // error has been reported by the `fdeflate` library.
    output.write_all(suffix).unwrap();
    let suffix_result = reader.next_frame(&mut image);
    let DecodingError::Format(suffix_err) = suffix_result.unwrap_err() else {
        panic!("Unexpected error variant");
    };
    let suffix_msg = format!("{suffix_err:?}");
    assert!(suffix_msg.contains("CorruptFlateStream"));
    assert!(suffix_msg.contains("InsufficientInput"));
}
